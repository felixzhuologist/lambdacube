...
pub fn typecheck(
    ...
    match term {
        ...
        Term::TyApp(box func, ty) => match typecheck(func, term_ctx)? {
            Type::All(s, box body) => Ok(body.clone().applysubst(&s, ty)),
            _ => Err(TypeError::TyFuncApp),
        },
        Term::Arith(box left, op, box right) => {
            match (typecheck(left, term_ctx)?, typecheck(right, term_ctx)?) {
        ...
        Term::If(box cond, box if_, box else_) => {
            ...
            if typecheck(cond, term_ctx)? != Type::Bool {
